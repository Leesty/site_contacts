...
# Фиче-флаг: создание чатов менеджерами через windowgram (кабинет «Прозвоны»/
# холодные контакты). ВЫКЛючен по решению владельца 2026-07-28 — временно, до
# отдельного распоряжения. Персональные права (`can_create_group_reports`) и
# привязки подадминов НЕ трогаем: включение обратно = этот флаг в true, доступ
# у всех 61 менеджера восстановится мгновенно, ничего не выдавая заново.
COLD_CHAT_CREATION_ENABLED = os.getenv("COLD_CHAT_CREATION_ENABLED", "false").lower() == "true"
...
# пользователей не трогаются. Сверено по БД 08.09.2026 (кластер 37.252.17.152).
#
# ВНИМАНИЕ: по бирже резюме тут не выводы, а НАЧИСЛЕНИЯ (решение владельца
# 08.09.2026). 1816 одобренных лидов × 25 ₽ = 45 400 ₽, из которых реально
# выведено только 600 ₽, а 44 800 ₽ до сих пор висят на балансах —
# в основном у madi_0395@mail.ru (1801 лид, 44 425 ₽).
#   review_exchange — 0 заявок на вывод (в LedgerEntry одно начисление на 5 ₽)
#   resume          — начислено 45 400 ₽, выведено 600 ₽
...
EXTERNAL_WITHDRAWALS = {
    ...
    "Биржа резюме": int(os.getenv("EXT_WD_RESUME", "45400")),
}
