...
class User(AbstractUser):
    ...
    # Заработок ДО перехода на новую систему (17.08.2026). Отдельный кошелёк
    # со своей кнопкой вывода и своей очередью: он не смешивается ни с новым
    # балансом (приоритетные выплаты), ни со старым легаси.
    old_balance = models.IntegerField(
        default=0,
        help_text="Баланс за период до 17.08.2026 (отдельный вывод).",
    )
    ...
    # Блокировка выплат за накрутку: аккаунт сам стартует свои же ссылки
    # (см. /staff/fraud/ — детектор). Ссылки и данные сохраняются, но синк
    # воронки не начисляет деньги, пока флаг стоит.
    fraud_blocked = models.BooleanField(
        default=False,
        db_index=True,
        help_text="Выплаты заблокированы за накрутку (самостарты своих ссылок).",
    )
    fraud_note = models.CharField(
        max_length=255, blank=True, default="",
        help_text="Причина блокировки выплат.",
    )
