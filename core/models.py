...
class User(AbstractUser):
    ...
    class Role(models.TextChoices):
        ...
        LID_CUSTOMER = "lid_customer", "Заказчик лидов (zavodlidov)"
    ...
    # DEPRECATED: право на «Прозвоны» теперь = can_create_group_reports
    # (одно общее право на 2 фичи). Поле оставлено для миграционной
    # совместимости, в логике не используется.
    can_create_call_reports = models.BooleanField(
        default=False,
        db_index=True,
        help_text="(deprecated) Используйте can_create_group_reports.",
    )
...
class LidProject(TimeStampedModel):
    ...
    Клиент создаёт проект (название + лимиты + первая порция собственных
    значений). Админ пополняет «пул» номеров для клиента (общий, не
    привязанный к проекту). Ежедневно в 11:00 МСК (lazy-fill при заходе
    в кабинет) каждый открытый проект пополняется случайными номерами
    из пула: `daily_limit` номеров за раз, до достижения `total_limit`.
    Когда `admin_items_count >= total_limit` → `is_closed=True`, новые
    раздачи не идут, можно скачать Excel.
    """
    ...
    business_date = models.DateField(
        db_index=True,
        help_text="Бизнес-дата создания проекта (MSK, cutoff 11:00).",
    )
    ...
    last_fill_business_date = models.DateField(
        null=True, blank=True,
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["customer", "is_closed"]),
        ]
...
class AdminPhonePool(TimeStampedModel):
    """Пул номеров от админа для конкретного клиента.

    Админ загружает большие пачки (до 10к+) в пул клиента. Каждый день
    в 11:00 МСК номера случайно разбираются по открытым проектам
    клиента согласно `daily_limit` каждого проекта (FIFO по
    created_at). Номер из пула используется однократно: после раздачи
    `is_used=True` + `used_in_project` указывает куда ушёл.
    """
    ...
    used_in_project = models.ForeignKey(
        "LidProject", on_delete=models.SET_NULL,
        null=True, blank=True,
    ...
    class Meta:
        ...
        ordering = ["created_at"]
        indexes = [
            models.Index(fields=["customer", "is_used"]),
        ]
        constraints = [
            models.UniqueConstraint(
